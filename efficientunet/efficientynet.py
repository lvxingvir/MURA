...
__all__ = ['EfficientYnet', 'get_efficientynet_b0', 'get_efficientynet_b1', 'get_efficientynet_b2',
           'get_efficientynet_b3', 'get_efficientynet_b4', 'get_efficientynet_b5']
...
class EfficientYnet(nn.Module):
    def __init__(self, encoder, n_classes=3,out_channels=2, concat_input=True):
        super().__init__()

        self.nclass = n_classes

        # feature_size = self.n_channels
        ...
        self.fc = nn.Linear(64, self.nclass, bias=False)
        self.out_nonlin = nn.Sigmoid()

        self.branch_classify = nn.Sequential(nn.Linear(self.n_channels,
                                                       64),
    ...
    def forward(self, x):
        ...
        x_flat = x_avg.view(batch_size, -1)
        final_features = self.branch_classify(x_flat)
...
def get_efficientynet_b0(out_channels=2, n_classes=3, concat_input=True, pretrained=True):
    encoder = EfficientNet.encoder('efficientnet-b0', pretrained=pretrained)
    model = EfficientYnet(encoder, n_classes=n_classes, out_channels=out_channels, concat_input=concat_input)
    return model

def get_efficientynet_b1(out_channels=2, n_classes=3, concat_input=True, pretrained=True):
    encoder = EfficientNet.encoder('efficientnet-b1', pretrained=pretrained)
    model = EfficientYnet(encoder, n_classes=n_classes, out_channels=out_channels, concat_input=concat_input)
    return model

def get_efficientynet_b2(out_channels=2, n_classes=3, concat_input=True, pretrained=True):
    encoder = EfficientNet.encoder('efficientnet-b2', pretrained=pretrained)
    model = EfficientYnet(encoder, n_classes=n_classes, out_channels=out_channels, concat_input=concat_input)
    return model

def get_efficientynet_b3(out_channels=2, n_classes=3, concat_input=True, pretrained=True):
    encoder = EfficientNet.encoder('efficientnet-b3', pretrained=pretrained)
    model = EfficientYnet(encoder, n_classes=n_classes, out_channels=out_channels, concat_input=concat_input)
    return model

def get_efficientynet_b4(out_channels=2, n_classes=3, concat_input=True, pretrained=True):
    encoder = EfficientNet.encoder('efficientnet-b4', pretrained=pretrained)
    model = EfficientYnet(encoder, n_classes=n_classes, out_channels=out_channels, concat_input=concat_input)
    return model

def get_efficientynet_b5(out_channels=2, n_classes=3, concat_input=True, pretrained=True):
